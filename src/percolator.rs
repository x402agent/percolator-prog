    ...
    pub fn process_instruction<'a, 'b>(
        ...
        match instruction {
            Instruction::InitMarket {
                ...
                // recovery call (PushHyperpMark + catchup_accrue)
                ...
                //       hyperp_mark_e6. The only way to populate that
                //       field is PushHyperpMark, which in turn requires a
                //       non-zero hyperp_authority AND oracle_price_cap_
            ...
            Instruction::InitUser { fee_payment } => {
                ...
                engine
                    .deposit_not_atomic(idx, capital_units as u128, 0, clock.slot)
                    .map_err(map_risk_error)?;
            ...
            Instruction::KeeperCrank {
                    ...
                    && engine.is_used(caller_idx as usize)
                {
                    ...
                    let mut reward = sweep_delta
                        ...
                        / 10_000u128;
                    // Cap reward by post-crank insurance balance. The old
                    // logic subtracted `insurance_floor` here; with the
                    // floor deleted, the cap is just the remaining
                    // insurance balance (which was what the floor-minus-
                    // balance gate effectively enforced once floor = 0).
            ...
            Instruction::SetOraclePriceCap { max_change_e2bps } => {
                ...
                // floor is set, disabling clamping would let PushHyperpMark
